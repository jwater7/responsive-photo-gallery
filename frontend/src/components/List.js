  ...
  <div style={{ paddingTop: '44px', height: '100%', textAlign: 'center' }}>
    <a
      href={
        ...
        '&imageIndex=' +
        encodeURIComponent(props.imageIndex)
      }
    >
      <div
        style={{
...
class List extends React.Component {
  ...
  componentDidMount() {
    if (
      !(this.props.match.params.album in this.props.list) ||
      !(this.props.match.params.album in this.props.collectionMap)
    ) {
      this.props.loadList(this.props.match.params.album, this.props.authtoken);
    }

    const { openAtCollection } = getURLParams(this.props.location.search, {
      openAtCollection: null,
    });
  ...
  photos = (collection, collectionItems = undefined) => {
    ...
    for (let i = 0; i < sortedCollectionItems.length; i++) {
      ...
      if (this.props.list[alb][filename].format === 'video') {
        ...
        delete imageobj['src'];
        imageobj['html'] = ReactDOMServer.renderToStaticMarkup(
          <VideoHtml
            thumburl={thumburl}
            basename={this.props.basename}
            album={this.props.match.params.album}
  ...
  render() {
    ...
    const { startIndex } = getURLParams(this.props.location.search, {
      startIndex: 0,
    });
    const collectionMap = this.getCollectionMapForAlbum(
      this.props.match.params.album
    );
    const collectionKeys = [
      ...(Object.keys(collectionMap).includes('favorites')
        ? ['favorites']
        : []),
              ...
              <div key={collectionKey}>
                <Link
                  ...
                  to={{
                    ...
                    search:
                      '?filter=' +
                      JSON.stringify(collectionMap[collectionKey].filter) +
                      '&description=' +
                      JSON.stringify(collectionMap[collectionKey].description),
                    /*search: '?filter=' + JSON.stringify({year: '2017', month: '3'}), */
                  }}
                >
                  <h5 style={{ overflow: 'hidden' }}>
                    {collectionMap[collectionKey].description}
                  </h5>
