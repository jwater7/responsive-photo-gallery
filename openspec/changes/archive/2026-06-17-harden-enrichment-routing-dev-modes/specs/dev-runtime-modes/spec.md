...
The stack must be runnable two documented ways that agree on where data lives:
a fast native hot-reload loop (Express + Next native, backends dockerized) and a
prod-like full docker-compose mode. In both, Express is the single backend, and
Next reaches it only through a single wildcard `/api/*`.
