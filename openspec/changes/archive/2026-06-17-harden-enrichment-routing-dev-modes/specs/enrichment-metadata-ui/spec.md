# enrichment-metadata-ui

A shared lightbox surface that exposes each image's enrichment (AI tags, OCR text,
place/coords, capture date, type/size) across album, search, and map, plus inline
favoriting — all fail-soft so the gallery keeps working when enrichment is down.
