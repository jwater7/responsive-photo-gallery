# enrichment-worker-process Specification

## Purpose
Defines the process topology of the enrichment plane: the heavy worker runs in its
own process so the API event loop (and its `/health` probe) stays responsive, with
triggers, progress, and scan state coordinated through Redis rather than in-process
memory.
